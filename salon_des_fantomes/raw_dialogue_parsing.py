...
# def text_to_elaborate(raw_dialogue_text):
#     """
#     gets the last thing someone said, which must be surrounded by quotes
#     """
#     utterances = re.findall(r'^((?:(?:[A-Z]\w+)(?:(?: [A-Za-z]\w+)){0,})(?:: \".+))(?:\")$',raw_dialogue_text,flags=re.M)
#     return utterances[-1]
...
def simple_regex_heal(raw_dialogue_text):
  ...
  if re.search(r'[.?!]',raw_dialogue_text)!=None: ## if has terminal punctuation somewhere
    ...
    ## "text 1" > "text 1...\""
    return re.sub(r' {0,}\"? {0,}?$','..."',raw_dialogue_text,count=1) ## ellipsis
